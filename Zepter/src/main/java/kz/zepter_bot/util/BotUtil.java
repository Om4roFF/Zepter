...
import kz.zepter_bot.dao.implement.KeyboardMarkUpDao;
import kz.zepter_bot.entity.standart.Message;
import kz.zepter_bot.util.type.ParseMode;
...
public class BotUtil {
    ...
    public int sendMessageWithoutKeyboard(String text,long chatId) throws TelegramApiException {
        return sendMessageWithoutKeyboard(text,chatId, 0);
    }

    private int sendMessageWithoutKeyboard(String text,long chatId,int replyMessageId) throws TelegramApiException{
        SendMessage sendMessage = new SendMessage()
                .setParseMode(ParseMode.html.name())
                .setChatId(chatId)
                .setText(text)
